  ...
  Future<void> getGPSLocation() async {
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Izin lokasi tidak dapat dipulihkan")));
        return;
      }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final response = await http.post(
          Uri.parse(
              'http://192.168.215.210/sertifikasi_jmp/backend/beli_roti.php'),
          body: {
            "nama": nama!,
            "email": email!,
        ...
        if (response.statusCode == 200) {
          ...
          if (result.contains("Pembelian berhasil")) {
            ScaffoldMessenger.of(context)
                .showSnackBar(SnackBar(content: Text("Pembelian berhasil")));
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(
                  "Pembelian gagal dengan status code: ${response.statusCode}")));
        }
      } catch (e) {
